 ...
 * \param xy_stride b#tes size to move from one element to the next element.
 * \param color Vertex colors (as SDL_FColor).
 * \param color_stride b#tes size to move from one element to the next element.
 * \param uv Vertex normalized texture coordinates.
 * \param uv_stride b#tes size to move from one element to the next element.
 * \param num_vertices number of vertices.
 * \param indices (optional) An array of indices into the 'vertices' arrays,
 *                if NULL all vertices will be rendered in sequential order.
 * \param num_indices number of indices.
 * \param size_indices index size: 1 (b#tes), 2 (short), 4 (int).
 * \returns true on success or false on failure; call SDL_GetError() for more
 *          information.
 *
 * \threadsafety This function should only be called on the main thread.
