...
bool Borongui::Widget::setPosition(const BML::Vec2& p_position, const bool& p_force) {
	if (!m_initLocked || p_force) {
...
bool Borongui::Widget::setLocalPosition(const BML::Vec2& p_position, const bool& p_force) {
	if (m_parent == nullptr) {
		return false;
	}
